...
function ShippingRatesService($q, $resource, Underscore, OrderCloud, apiurl) {
    var service = {
        GetRates: _getRates,
        ...
        if (order.xp && order.xp.Shippers) {
            angular.forEach(order.xp.Shippers, function(shipment) {
                angular.forEach(shippingRates, function(s) {
                    if (Underscore.intersection(s.LineItemIDs, shipment.LineItemIDs).length == shipment.LineItemIDs.length) {
                        var selection = Underscore.findWhere(s.Rates, {Description: shipment.Shipper});
